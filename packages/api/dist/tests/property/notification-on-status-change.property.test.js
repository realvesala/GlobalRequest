"use strict";
// Feature: lab-measurement-request-system, Property 15: Notification Creation on Status Change
/**
...
(0, vitest_1.describe)('Property 15: Notification Creation on Status Change', () => {
    (0, vitest_1.it)('for In_Progress transition, notifications are created for requestor, technician and manager (>=100 iterations)', async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 30 }), async (material) => {
            ...
            await (0, supertest_1.default)(app)
                ...
                .send({ lab_id: labId });
            const toProgress = await (0, supertest_1.default)(app)
                .post(`/requests/${requestId}/assign-technician`)
                .set('X-Mock-User-Id', managerId)
                .send({ technician_id: technicianId });
            (0, vitest_2.expect)(toProgress.status).toBe(200);
            (0, vitest_2.expect)(toProgress.body.status).toBe('In_Progress');
            const rows = db
                .prepare(`SELECT user_id, event_type, message
               FROM notifications
               WHERE request_id = ?`)
                .all(requestId);
            const recipients = new Set(rows.map((r) => r.user_id));
            (0, vitest_2.expect)(recipients.has(requestorId)).toBe(true);
            (0, vitest_2.expect)(recipients.has(technicianId)).toBe(true);
            (0, vitest_2.expect)(recipients.has(managerId)).toBe(true);
            const statusChangedRows = rows.filter((r) => r.event_type === 'status_changed');
            (0, vitest_2.expect)(statusChangedRows.length).toBeGreaterThanOrEqual(3);
        }), { numRuns: 100 });
    }, 120000);
});
