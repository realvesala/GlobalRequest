...
(0, vitest_1.describe)('Property 11: Technician Assignment and Status Invariant', () => {
    (0, vitest_1.it)('assign -> In_Progress, reassign keeps In_Progress (>=100 iterations)', async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 40 }), async (material) => {
            ...
            (0, vitest_2.expect)(created.status).toBe(201);
            const requestId = created.body.id;
            const assigned = await (0, supertest_1.default)(app)
                .post(`/requests/${requestId}/assign`)
                .set('X-Mock-User-Id', managerId)
                .send({ lab_id: labId });
            (0, vitest_2.expect)(assigned.status).toBe(200);
            ...
            (0, vitest_2.expect)(toProgress.body.assigned_technician_id).toBe(techA);
            const reassigned = await (0, supertest_1.default)(app)
                .post(`/requests/${requestId}/reassign-technician`)
                .set('X-Mock-User-Id', managerId)
                .send({ technician_id: techB });
            (0, vitest_2.expect)(reassigned.status).toBe(200);
            (0, vitest_2.expect)(reassigned.body.status).toBe('In_Progress');
            (0, vitest_2.expect)(reassigned.body.assigned_technician_id).toBe(techB);
        }), { numRuns: 100 });
    }, 90000);
});
