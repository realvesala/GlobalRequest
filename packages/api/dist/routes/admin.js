...
function createAdminRouter(db) {
    ...
    router.get('/labs', (_req, res) => {
        ...
        const getMethods = db.prepare(`SELECT m.* FROM methods m
    ...
    router.post('/labs', (req, res) => {
        const { name, region, contact_info, method_ids } = req.body;
        if (!name || !region) {
       ...
       VALUES (?, ?, ?, ?, 1, ?, ?)`).run(id, name, region, contact_info ?? null, now, now);
        if (Array.isArray(method_ids) && method_ids.length > 0) {
            const insertLm = db.prepare('INSERT OR IGNORE INTO lab_methods (lab_id, method_id) VALUES (?, ?)');
    ...
    router.put('/labs/:id', (req, res) => {
       ...
       WHERE id = ?`).run(name ?? existing.name, region ?? existing.region, contact_info !== undefined ? contact_info : existing.contact_info, now, id);
    ...
    router.post('/methods', (req, res) => {
        const { name, description, required_material } = req.body;
        if (!name) {
       ...
       VALUES (?, ?, ?, ?, 1, ?, ?)`).run(id, name, description ?? null, required_material ?? null, now, now);
    ...
    router.put('/methods/:id', (req, res) => {
        ...
        const { name, description, required_material } = req.body;
        const now = new Date().toISOString();
        db.prepare(`UPDATE methods SET
        name = ?, description = ?, required_material = ?, updated_at = ?
       WHERE id = ?`).run(name ?? existing.name, description !== undefined ? description : existing.description, required_material !== undefined ? required_material : existing.required_material, now, id);
