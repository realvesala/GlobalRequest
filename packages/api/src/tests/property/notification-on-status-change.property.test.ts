...
function seedUser(
  ...
  role: 'Requestor' | 'Lab_Manager' | 'Lab_Technician'
): string {
  const id = uuidv4();
...
describe('Property 15: Notification Creation on Status Change', () => {
  it(
    'for In_Progress transition, notifications are created for requestor, technician and manager (>=100 iterations)',
    async () => {
      await fc.assert(
        fc.asyncProperty(fc.string({ minLength: 1, maxLength: 30 }), async (material) => {
          ...
          const rows = db
            .prepare(
              `SELECT user_id, event_type, message
               FROM notifications
               WHERE request_id = ?`
            )
            .all(requestId) as Array<{ user_id: string; event_type: string; message: string }>;

          const recipients = new Set(rows.map((r) => r.user_id));
          expect(recipients.has(requestorId)).toBe(true);
          expect(recipients.has(technicianId)).toBe(true);
          expect(recipients.has(managerId)).toBe(true);

          const statusChangedRows = rows.filter((r) => r.event_type === 'status_changed');
          expect(statusChangedRows.length).toBeGreaterThanOrEqual(3);
        }),
        { numRuns: 100 }
      );
    },
